...
- 关于 Topology 策略参考: [Topology](../end-user/policies/references.md#topology)
- 关于 Deploy 步骤参考: [Deploy](../end-user/workflow/built-in-workflow-defs.md#deploy)
...
如果你开启了 [`velaux`](../reference/addons/velaux.md) 插件，你可以直接在 UI 控制台完成 Kubernetes 对象的部署。

### 规划并创建交付目标

[交付目标(Target)](../getting-started/core-concept.md#交付目标（target）) 定义了应用交付的运行时集群和命名空间，创建交付目标的同时完成运行时集群的命名空间创建。
...
完成交付目标创建后，我们开始创建应用。与 [交付第一个应用](../quick-start.md) 一样，首先我们需要填写应用的基础信息，这里有三个不同点：
...
如果你想了解如何将 Kubernetes 对象分发至多集群，可以参考[多集群部分](../end-user/components/ref-objects.md)文档。
