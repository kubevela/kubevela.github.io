...
- 一个新的插件需要需要先提交到 `experimental` 目录，作为试验阶段的插件。如果你的插件已经在生产环境中经过了大量的测试和验证，并被多数维护者评审通过，也可以直接作为正式阶段的插件。
  
- 一个试验阶段的插件通常需要具备以下条件，可以提升成为正式阶段的插件：
  
  - 该插件被添加到了仓库的 [e2e-test](https://github.com/kubevela/catalog/tree/master/test/e2e-test/addon-test) 中，从而保证插件能够被正常启用。
    
  - 插件必须包含以下基本的信息：
     
    - 元数据文件中（`metadata.yaml`）中包含了一个可达的图标 url 和插件源 url。
