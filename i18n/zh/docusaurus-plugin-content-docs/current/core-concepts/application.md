...
> 注意：由于你没有声明放置规则，KubeVela 会将此应用程序直接部署到控制平面集群（即你的 `kubectl` 正在与之通信的集群）。 如果你使用 KinD 或 MiniKube 等本地集群来玩 KubeVela，也是同样的情况。
...
同样，系统将根据你设置的属性分配一个 HPA 实例并将其“链接”到目标工作负载实例，组件本身不受影响。
