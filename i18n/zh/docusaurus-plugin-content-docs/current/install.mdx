...
* KubeVela 的 dashboard 插件
  ```shell
  vela addon enable velaux
  ```

  插件安装成功之后，可以通过 `vela port-forward` 命令做端口映射，注意选择 velaux 组件的 service，将 dashboard 的端口映射到本地。例如，我们以指定 `18080` 端口为例：

  ```shell
  vela port-forward -n vela-system addon-velaux 18080:80
  ```

  通过访问浏览器访问本地端口 `http://127.0.0.1:18080` 可以访问到 dashboard 页面.
