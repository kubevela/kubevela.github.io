...
```shell script
curl -fsSl https://kubevela.io/script/install.sh | bash -s 1.3.0-beta.2
```
...
helm repo update
helm install --create-namespace -n vela-system kubevela kubevela/vela-core --version 1.3.0-beta.2 --wait
```
...
```shell script
vela addon enable velaux serviceType=NodePort --version 1.3.0-beta.2

# 获取映射的端口，通过该端口访问 VelaUX
kubectl get service velaux -n vela-system -o jsonpath="{.spec.ports[0].nodePort}"
```

VelaUX 有鉴权机制，默认账号为admin，通过以下方式获取密码
