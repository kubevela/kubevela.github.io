...
本文档向你展示如何在远程服务器（如 aws EC2 或 aliyun ECS）上以 all-in-one 模式安装 KubeVela。

## 1.准备一台机器

你可以在 Linux、macOS 或 Windows 上安装 KubeVela。 通常我们使用 Linux 作为远程服务器，本文档将以 Linux 为例。

**要求**

1. 确保你的机器安装了`curl`。
2. 此服务器有一个公网IP，或者在企业内网中你可以访问该服务器的IP，下文统一称为有公网IP。
3. 如果你使用的是云服务器，请确保你在安全组中打开了 6443 端口。

:::note
如果服务器没有公网IP，可以继续安装。 在这种情况下，你只通过ssh的方式登陆服务器访问 KubeVela。而且你无法访问 VelaUX。
:::
...
根据你的服务器有无公网IP，执行下面的命令安装 KubeVela。

<Tabs groupId="public-ip">
  <TabItem value="with" label="有公网IP">
  ...
  <TabItem value="without" label="没有公网IP">
...
如果服务器有公共 IP，你可以使用本地计算机访问 KubeVela。 请确认在上一步服务器上执行 `velad install` 时加了 `--bind-ip` 参数。
...
要访问集群，您还需要在本地机器上安装 vela CLI (`vela`)。
