...
1. [本地机器](#local)：在本地机器中安装
2. [远程服务器](#remote)：在具有可访问IP的远程服务器中安装

:::caution
以独立模式安装仅适用于开发和测试目的。 不建议在生产环境中使用。
:::
...
默认情况下，velaux 没有任何暴露的端口，期望可以通过以下方式查看：
...
下面向你展示如何在远程服务器（如 aws EC2 或 aliyun ECS）上以独立模式安装 KubeVela。
...
1. 确保你的机器安装了`curl`。
2. 此服务器有一个公网IP，或者在企业内网中你可以访问该服务器的IP。
