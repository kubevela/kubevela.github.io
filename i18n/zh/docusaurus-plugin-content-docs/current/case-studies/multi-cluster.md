...
工作流会先灰度发布预发环境，执行完第一步之后工作流进入暂停阶段，你可以运行下面的命令来查看应用的状态。
```shell
kubectl get app example-app-rollout
NAME                  COMPONENT            TYPE         PHASE               HEALTHY      STATUS       AGE
example-app-rollout   hello-world-server   webservice   workflowSuspending    true      Ready:2/2    9m7s
```
你可以通过检查 `status.service` 字段来确认预发环境的工作负载的两个副本均已就绪。
...
```yaml
    ...
    - env: staging
      ...
      - apiVersion: core.oam.dev/v1alpha2
        ...
        name: health-policy-demo
        namespace: default
        uid: ebf717e2-4d84-47dc-968f-fc23e7c5e330
      traits:
      - healthy: true
        type: rollout
      workloadDefinition:
...
通过工作流的 resume 指令，继续将应用灰度发布到生产环境。

```shell
> vela workflow resume example-app-rollout
Successfully resume workflow: example-app-rollout
```
...
kubectl get app example-app-rollout
NAME                  COMPONENT            TYPE         PHASE     HEALTHY   STATUS       AGE
example-app-rollout   hello-world-server   webservice   running   true      Ready:5/5    9m7s
```

再次查看 `status.service` 字段可以确认生产环境的五个副本也已全部就绪。
      ...
      - env: prod
        ...
        scopes:
          - apiVersion: core.oam.dev/v1alpha2
            ...
            name: health-policy-demo
            namespace: default
            uid: 5a67e803-a543-4a79-ac7f-8610b5b6024f
        traits:
          - healthy: true
            type: rollout
        workloadDefinition:
