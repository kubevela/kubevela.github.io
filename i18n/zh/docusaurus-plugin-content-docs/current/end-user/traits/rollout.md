...
下面解释了此过程是 **如何发生** 的，在使用了 `kruise-rollout` 特性升级配置后，整个过程会分成3步：

1. 当开始升级时，会产生 `20%` 的实例数量。在我们的示例中， 我们一共设置了5个实例，它会保留所有的老的版本并创建 `5 * 20% = 1` 个金丝雀实例版本，并且导入了 `20%` 的流量。在所有都准备好后，它会等待手工批准。
...
再次访问网关，你会发现有 `20%` 的机率获取结果是 `Demo: v2`。
...
用户通过业务的相关指标，如：日志、Metrics等其它手段，验证金丝雀的版本访问成功后，你可以继续执行工作流继续后续过程。

```shell
vela workflow resume canary-demo
```

在多次重新访问网关后，你会发现机率大幅提升，有 (`90%`) 机率满足了 `Demo: v2`。
