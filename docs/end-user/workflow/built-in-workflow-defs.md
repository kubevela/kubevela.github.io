...
> The ConfigMap is like below: the `name` and `namespace` of the ConfigMap is the same in `properties`. In `data`, the key is `name`, and the value is the yaml of the deployed resource.
> ```yaml
> apiVersion: v1
> kind: ConfigMap
> metadata:
