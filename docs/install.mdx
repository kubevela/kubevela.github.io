...
## 5. [Optional] Enable Multi-Cluster Management

KubeVela provides the capability of orchestrating applications across different kubernetes clusters. This capability can be enabled by running the following command

```shell
helm upgrade --install kubevela kubevela/vela-core -n vela-system --set multicluster.enabled=true --create-namespace --wait
```

You can verify it by running `vela cluster list`.

```shell
> vela cluster list
No managed cluster found. 
```
