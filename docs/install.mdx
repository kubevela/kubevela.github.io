...
* Dashboard addon
  ...
  ```
  After dashboard addon enabled successfully, you can use `vela port-forward` command and select the service of component `velaux` to use the dashboard.


  For example, let's specify the local port `18080`:
  ```shell
  vela port-forward -n vela-system addon-velaux 18080:80
  ```
  Then you can visit it by http://127.0.0.1:18080.
