...
If your Kubernetes cluster has cloud LoadBalancer available, you can configure the `serviceType` for `velaux` addon.
Configure it as `serviceType=LoadBalancer`, and obtain the access address through the following methods after activation:

```shell script
vela addon enable velaux serviceType=LoadBalancer
```

View the endpoint by:
