...
You need to create an addon directory to place addon resource files. Won't bother to create? `vela addon init` command will do this for you. You can create a basic structure or with a Helm component inside. Please refer to `vela addon init -h` for details. For quick starts, we will just use `vela addon init your-addon-name` now.
