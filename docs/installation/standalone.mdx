...
1. [Local machine](#local): Install on a local machine.
2. [Remote Linux server](#remote): Install on a remote Linux server with accessible IP.

:::caution
Installation with standalone mode is suited for development and testing purpose only. It is not recommended to use it in production environment.
:::

## Install on a local machine {#local}

### 1. Prepare environment

You can install KubeVela on machines with these OS: Linux, macOS, Windows。
...
[VelaD](https://github.com/kubevela/velad) packages KubeVela with all dependencies for minimal installation and VelaUX, which enables you to run KubeVela on any of docker compatible environments or linux system.
...
1. Make sure your machine have `curl` installed.
2. This server have a public IP, or you can access the server's IP in the corporate intranet.
