...
```shell
curl -fsSl https://kubevela.io/script/install-velad.sh | bash -s 1.7.6
```
...
- A file uesd to configure access to clusters is called _kubeconfig file_
  :::
...
- A file uesd to configure access to clusters is called _kubeconfig file_. Vela CLI also use this file to access KubeVela.
  :::
