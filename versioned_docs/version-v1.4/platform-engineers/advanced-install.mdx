...
```shell script
curl -fsSl https://kubevela.net/script/install.sh | bash -s 1.4.3
```
...
helm repo update
helm install --create-namespace -n vela-system kubevela kubevela/vela-core --version 1.4.3 --wait
```
...
```shell script
curl -fsSl https://kubevela.io/script/install.sh | bash -s 1.4.3
```
